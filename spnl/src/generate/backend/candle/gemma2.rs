...
pub struct Gemma2GenericConfig {
...
impl From<Gemma2GenericConfig> for Gemma2Config {
    fn from(config: Gemma2GenericConfig) -> Self {
        let hidden_activation = match config.hidden_act.as_deref() {
            ...
            _ => candle_nn::Activation::GeluPytorchTanh, // Gemma2 default
        };
...
pub struct Gemma2ModelWrapper {
    model: Gemma2Model,
    config: Gemma2Config,
    cache_position: usize, // Track current cache length for reuse
}

impl Gemma2ModelWrapper {
    pub fn load(vb: VarBuilder, config: Gemma2Config) -> anyhow::Result<Self> {
...
impl ModelForward for Gemma2ModelWrapper {
...
impl CandleModel for Gemma2ModelWrapper {
    fn generate(
        &mut self,
        tokens: &[u32],
    ...
    fn eos_token_id(&self, tokenizer: &Tokenizer) -> u32 {
        // Try to get EOS token from tokenizer, fallback to common Gemma2 EOS token
        tokenizer
            .token_to_id("<eos>")
            .or_else(|| tokenizer.token_to_id("</s>"))
            .or_else(|| tokenizer.token_to_id("<end_of_turn>"))
            .unwrap_or(1) // Gemma2 typically uses 1 as EOS
    }
}
