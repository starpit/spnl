...
pub fn demo(args: Args) -> Unit {
    ...
    spnl!(g model (cross (desc "Ask the model to select the best option from the candidates")
          ...
          (plusn n
                 (desc (format "Generate {n} candidate emails in parallel"))
                 (g model (format "write an introductory email for a job application, limited to at most {max_tokens} characters. use your imagination, go wild.")
